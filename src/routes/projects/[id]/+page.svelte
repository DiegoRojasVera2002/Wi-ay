<script>
	...
	import { getProject, getProjectSteps, enrollInProject, submitCode, getUser } from '$lib/api';
...
<div class="lesson-page-duo">
	...
	{:else}
		...
		<div class="lesson-container">
			{#if steps.length > 0}
				<div class="lesson-content" class:shake-animation={shakeError}>
					<!-- Step indicator -->
					<div class="step-indicator">
						<span class="step-label">Paso {currentStep + 1} de {steps.length}</span>
					</div>

					<!-- Question/Task card -->
					<div class="task-card-duo">
					...
					<div class="code-section-duo">
						<div class="code-header">
							...
							<span class="code-label">Escribe tu código en Python</span>
						</div>
						<textarea
							bind:value={code}
					...
					<!-- Result feedback -->
					{#if result}
						<div class="result-feedback-duo" class:success={result.status === 'passed'} class:error={result.status !== 'passed'}>
							...
							{#if result.results && result.results.length > 0}
								<div class="test-list-duo">
									{#each result.results as test}
										<div class="test-row-duo" class:passed={test.status === 'passed'}>
											<span class="test-check">{test.status === 'passed' ? '✓' : '✗'}</span>
											<span class="test-name">Test {test.test}</span>
											{#if test.expected}
												<div class="test-details">
													<span class="expected">Esperado: {test.expected}</span>
				...
				<div class="lesson-bottombar">
					<div class="bottombar-content">
						{#if result && result.status === 'passed'}
							...
							<button class="btn-lesson-duo btn-continue" on:click={nextStep} disabled={currentStep === steps.length - 1}>
			...
			{:else}
				<div class="empty-lesson">
					...
					<h3>No hay pasos disponibles</h3>
					<p>Este proyecto aún no tiene pasos configurados</p>
					<a href="/modules" class="btn-primary">VOLVER A MÓDULOS</a>
				</div>
			{/if}
		</div>
